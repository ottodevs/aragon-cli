            ...
            task: async ctx => {
              ...
              try {
                ...
                if (apm.validInitialVersions.indexOf(ctx.version) == -1) {
                  throw new Error(
                    "Invalid initial version, it can only be 0.0.1, 0.1.0 or 1.0.0. Check your arapp file."
                  );
                } else {
              ...
              if (ctx.version == repo.version) {
                throw new Error(
                  "Version is already published, please bump it using `aragon apm version [major, minor, patch]`"
                );
              }

              const isValid = await apm.isValidBump(
                module.appName,
                repo.version,
                ctx.version
              );
    ...
    },
    {
      ...
      task: async (ctx, task) => {
        ...
        if (onlyContent && !pathExistsSync(`${dir}/${ARTIFACT_FILE}`)) {
          return taskInput(
            "Couldn't find artifact.json, do you want to generate one? [y]es/[a]bort",
            {
              validate: value => {
